...
const filePath = 'src/presentation/components/admin/AddLectureModal.tsx';
...
// 2. Main Wrapper
const oldWrapperStart = `    if (!isOpen) return null;
...
// 3. Transitions
const oldStep1Form = `                    {step === 1 ? (
                        <form id="detailsForm" onSubmit={handleSubmitDetails} className="space-y-4">`;
const newStep1Form = `                    <AnimatePresence mode="wait">
                        {step === 1 ? (
                            <motion.form
                                key="step1"
                                ...
                                className="space-y-6"
                            >`;
content = content.replace(oldStep1Form, newStep1Form);

const oldStep2Div = `                    ) : (
                        <div className="space-y-6">`;
const newStep2Div = `                        </motion.form>
                        ) : (
                            <motion.div
                                key="step2"
                                initial={{ opacity: 0, x: 20 }}
                            ...
                            >`;
content = content.replace(oldStep2Div, newStep2Div);

// 4. Inputs
content = content.replace(
    /className="w-full h-10 px-3 rounded-lg border border-slate-200 focus:border-blue-500 outline-none transition-colors"/g,
...
content = content.replace(
    /className="w-full h-10 px-3 rounded-lg border border-slate-200 focus:border-blue-500 outline-none transition-colors appearance-none bg-white"/g,
...
content = content.replace(
    /className="w-full h-10 px-3 rounded-lg border border-slate-200 focus:border-blue-500 outline-none transition-colors appearance-none bg-white disabled:bg-slate-50 disabled:cursor-not-allowed"/g,
...
content = content.replace(
    /className="w-full p-3 rounded-lg border border-slate-200 focus:border-blue-500 outline-none transition-colors resize-none"/g,
...
content = content.replace(
    /className="w-full h-10 pr-10 pl-3 rounded-lg border border-slate-200 focus:border-blue-500 outline-none transition-colors text-sm"/g,
...
// 5. Lecture Type Cards
const oldLiveCard = `                                        <button
                                            type="button"
...
content = content.replace(oldRecordedCard, newRecordedCard);

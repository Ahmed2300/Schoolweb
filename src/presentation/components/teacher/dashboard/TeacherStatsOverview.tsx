        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-8">
            {statsCards.map((stat, index) => (
                <motion.div
                    ...
                    className="relative group bg-white rounded-2xl p-5 border border-slate-100 shadow-sm hover:shadow-md transition-all duration-300 overflow-hidden"
                >
                    ...
                    <div className="relative z-10 flex justify-between items-start mb-4">
                        ...
                        <div className={`w-10 h-10 rounded-xl ${stat.color === 'shibl-crimson' ? 'bg-shibl-crimson/10 text-shibl-crimson group-hover:bg-shibl-crimson/20' : `bg-${stat.color}-50 text-${stat.color}-600 group-hover:bg-${stat.color}-100`} flex items-center justify-center transition-colors`}>
                            <stat.icon size={20} />
                        </div>
                    </div>
                    ...
                    <div className="relative z-10 flex items-center gap-2">
                        {stat.change ? (
                            <span className={`text-xs font-medium px-2 py-0.5 rounded-full flex items-center gap-1 ${stat.changeType === 'positive' ? 'bg-emerald-50 text-emerald-600' : 'bg-slate-100 text-slate-600'
                                }`}>
                                {stat.changeType === 'positive' && <TrendingUp size={10} />}
                                {stat.change}
                            </span>
                        ) : (
                            <span className="text-xs text-slate-grey">لا يوجد تغيير</span>
                        )}
                    </div>
                </motion.div>
            ))}

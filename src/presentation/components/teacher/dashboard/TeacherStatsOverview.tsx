    ...
    if (isLoading) {
        return (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-8">
                {[1, 2, 3].map((i) => (
                    <div key={i} className="bg-white dark:bg-slate-900 rounded-2xl p-5 border border-slate-100 dark:border-slate-800 shadow-sm h-32 animate-pulse">
                        <div className="flex justify-between items-start">
                            <div className="h-4 w-20 bg-slate-100 dark:bg-slate-800 rounded mb-3"></div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-8">
            {statsCards.map((stat, index) => (
                <motion.div
                    ...
                    className="relative group bg-white dark:bg-slate-900 rounded-2xl p-5 border border-slate-100 dark:border-slate-800 shadow-sm hover:shadow-md transition-all duration-300 overflow-hidden"
                >
                    ...
                    <div className="relative z-10 flex justify-between items-start mb-4">
                        <div>
                            <p className="text-slate-500 dark:text-slate-400 text-xs font-medium mb-1">{stat.title}</p>
                    ...
                    <div className="relative z-10 flex items-center gap-2">
                        {stat.change ? (
                            <span className={`text-xs font-medium px-2 py-0.5 rounded-full flex items-center gap-1 ${stat.changeType === 'positive' ? 'bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600 dark:text-emerald-400' : 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300'
                                }`}>
                                {stat.changeType === 'positive' && <TrendingUp size={10} />}
                                {stat.change}
                            </span>
                        ) : (
                            <span className="text-xs text-slate-grey dark:text-slate-500">لا يوجد تغيير</span>
                        )}
                    </div>
                </motion.div>
            ))}

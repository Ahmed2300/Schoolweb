        ...
        return (
            <div className="flex gap-2 overflow-x-auto pb-2">
                {[...Array(7)].map((_, i) => (
                    <div
                        key={i}
                        className="h-16 w-24 shrink-0 animate-pulse rounded-lg bg-gray-200"
                    />
                ))}
            </div>
        );
            ...
            {days.map((day) => {
                ...
                return (
                    <button
                        ...
                        `}
                    >
                        <span className={`text-sm font-semibold ${isSelected ? 'text-shibl-crimson' : 'text-gray-700'}`}>
        ...
        <div className={`${cardBaseStyles} ${stateStyles}`}>
            ...
            <div className="flex items-start justify-between mb-4">
                <div className="flex items-center gap-2">
                    <div className={`
                        p-2 rounded-xl 
                        ${slot.is_mine ? 'bg-emerald-100 text-emerald-600' :
                            slot.is_available ? 'bg-shibl-crimson/5 text-shibl-crimson' :
                ...
                {/* Badges */}
                {slot.is_mine && (
                    <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-emerald-100 border border-emerald-200 shadow-sm">
                        <Check className="w-3.5 h-3.5 text-emerald-600" />
                        <span className="text-[10px] font-bold text-emerald-700">حجزك</span>
                    </div>
                )}
            </div>
            ...
            <div className="mt-auto">
                {slot.is_mine ? (
                    <button
                        ...
                        disabled={isLoading}
                        className="
                            w-full flex items-center justify-center gap-2 py-2.5 rounded-xl border-2 border-red-100 
                            text-red-500 font-bold text-sm transition-all duration-200
                            hover:bg-red-50 hover:border-red-200 active:scale-[0.98]
                    ...
                    <div className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl bg-gray-50 border border-gray-100 text-gray-400 font-medium text-sm cursor-not-allowed">
...
function TimeSlotCardSkeleton() {
    return (
        <div className="flex flex-col justify-between rounded-2xl border border-gray-100 bg-white p-4 h-[180px]">
            {/* Header Skeleton */}
            <div className="flex items-start justify-between mb-4">
                <div className="flex items-center gap-2">
...
function ScheduleSummary({ slots, isLoading, onCancel }: ScheduleSummaryProps) {
    const statusBadge = (status: string) => {
        switch (status) {
            case 'approved':
                return (
                    <span className="flex items-center gap-1.5 text-[10px] font-bold px-2 py-1 rounded-full bg-emerald-100 text-emerald-700 border border-emerald-200">
            ...
            case 'pending':
                return (
                    <span className="flex items-center gap-1.5 text-[10px] font-bold px-2 py-1 rounded-full bg-amber-100 text-amber-700 border border-amber-200">
            ...
            case 'rejected':
                return (
                    <span className="flex items-center gap-1.5 text-[10px] font-bold px-2 py-1 rounded-full bg-red-100 text-red-700 border border-red-200">
    ...
    if (slots.length === 0) {
        return (
            <div className="flex flex-col items-center justify-center gap-4 py-8 text-gray-400 rounded-xl border-2 border-dashed border-gray-100">
                <div className="p-3 bg-gray-50 rounded-full">
                    <Calendar className="h-8 w-8 opacity-50" />
                </div>
                <div className="text-center">
                    <p className="font-medium text-gray-900">لا توجد مواعيد محجوزة</p>
    ...
    return (
        <div className="space-y-3">
            {slots.map((slot) => (
                <div
                    key={slot.id}
                    className="group flex items-start justify-between gap-3 rounded-xl border border-gray-100 bg-white p-3 hover:shadow-md hover:border-gray-200 transition-all duration-200"
                >
        ...
        <div className="space-y-6 p-4 md:p-6" dir="rtl">
            ...
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    ...
                    <p className="text-gray-600">اختر مواعيد حصصك الأسبوعية المتكررة</p>
                </div>
                <button
            ...
            <div className="flex flex-wrap items-center gap-4 rounded-xl border border-gray-200 bg-white p-4">
                <div className="flex items-center gap-2">
                    ...
                    <select
                        ...
                        className="w-[180px] px-3 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-shibl-crimson focus:border-shibl-crimson outline-none"
                    >
                ...
                <div className="flex items-center gap-2">
                    ...
                    <select
                        ...
                        className="w-[180px] px-3 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-shibl-crimson focus:border-shibl-crimson outline-none disabled:bg-gray-50 disabled:text-gray-400"
                    >
                        {isLoadingSemesters ? (
            ...
            <div className="rounded-xl border border-gray-200 bg-white p-4">
                <h2 className="mb-4 font-semibold text-gray-900">أيام الأسبوع</h2>
                <WeekDayTabs
                    days={weekConfig}
            ...
            <div className="grid gap-6 lg:grid-cols-3">
                ...
                <div className="lg:col-span-2">
                    <div className="rounded-xl border border-gray-200 bg-white p-4">
                        <h2 className="mb-4 font-semibold text-gray-900">المواعيد المتاحة</h2>
                ...
                <div className="lg:col-span-1">
                    <div className="rounded-xl border border-gray-200 bg-white p-4 sticky top-4">
                        <h2 className="mb-4 font-semibold text-gray-900">حجوزاتي</h2>
                        <ScheduleSummary
                            slots={mySchedule}

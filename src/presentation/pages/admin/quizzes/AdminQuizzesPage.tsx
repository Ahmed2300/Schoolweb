    ...
    return (
        <div className="p-6 max-w-[1600px] mx-auto space-y-6">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h1 className="text-2xl font-bold text-slate-900 dark:text-white">إدارة الاختبارات</h1>
                    <p className="text-slate-500 dark:text-slate-400 mt-1">مراجعة واعتماد اختبارات المدرسين</p>
                </div>
            </div>

            {/* Filters */}
            <div className="bg-white dark:bg-[#1E1E1E] p-4 rounded-xl shadow-sm border border-slate-200 dark:border-white/10 flex flex-wrap gap-4 items-center">
                    ...
                    {[
                        ...
                        <button
                            ...
                            className={`px-4 py-2 rounded-md text-sm font-medium transition-all ${selectedStatus === status.id
                                ? 'bg-white dark:bg-[#1E1E1E] text-indigo-600 dark:text-indigo-400 shadow-sm'
                ...
                <div className="relative flex-1 min-w-[200px]">
                    ...
                    <input
                        ...
                        className="w-full pr-10 pl-4 py-2 bg-slate-50 dark:bg-[#2A2A2A] border border-slate-200 dark:border-white/10 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 dark:text-white"
                    />
                </div>
            </div>

            {/* Content */}
            <div className="bg-white dark:bg-[#1E1E1E] rounded-xl shadow-sm border border-slate-200 dark:border-white/10 overflow-hidden">
                {isLoading ? (
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full text-right">
                            <thead className="bg-slate-50 dark:bg-[#2A2A2A] border-b border-slate-200 dark:border-white/10 text-xs uppercase text-slate-500 dark:text-slate-400 font-semibold">
                                <tr>
                            ...
                            <tbody className="divide-y divide-slate-100 dark:divide-white/10">
                                {quizzes.map((quiz) => (
                                    <tr key={quiz.id} className="hover:bg-slate-50/50 dark:hover:bg-white/5 transition-colors">
                                        <td className="px-6 py-4">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                    <div className="font-semibold text-slate-900 dark:text-white">{getTranslatedName(quiz.name)}</div>
                                                    <div className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">#{quiz.id}</div>
                                                </div>
                                            </div>
                                        </td>
                                        <td className="px-6 py-4">
                                        ...
                                        <td className="px-6 py-4">
                                            <div className="text-sm text-slate-600">
                                                <span className="inline-flex items-center gap-1 bg-slate-100 dark:bg-white/10 px-2 py-0.5 rounded text-xs dark:text-slate-300">
                                        ...
                                        <td className="px-6 py-4">
                                            <div className="flex justify-end gap-2">
                                                <button
                                                    ...
                                                    className="p-2 text-indigo-600 dark:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-500/10 rounded-lg transition-colors flex items-center gap-1.5 text-sm font-medium"
                                                >
                                                    <Eye size={16} />
                                                    عرض
                                                </button>
                    ...
                    <div className="p-16 text-center">
                        ...
                        <h3 className="text-lg font-medium text-slate-900 dark:text-white">لا توجد اختبارات</h3>
                        <p className="text-slate-500 dark:text-slate-400 mt-1">لم يتم العثور على اختبارات في هذه القائمة.</p>
                    </div>
                )}

                {/* Pagination (Simple Implementation) */}
                {totalPages > 1 && (
                    <div className="p-4 border-t border-slate-200 dark:border-white/10 flex justify-center gap-2">
                        <button
                            disabled={page === 1}
                            onClick={() => setPage(p => Math.max(1, p - 1))}
                            className="px-3 py-1 border border-slate-200 dark:border-white/10 rounded disabled:opacity-50 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-white/5 transition-colors"
                        >
                            السابق
                        </button>
